%[text] # Arbeitspaket 4: Detaillierte Abbildung der EE-Anlagen
%%
%[text] ## Eingangsdaten
...
ap4 = optimproblem("Description", "AP4", "ObjectiveSense", "min");
%%
...
% Korrektur Windkraft Onshore (Outdated)
...
ap4.Objective = sum(sum(c_var_matrix .* p + c_fix_matrix .* v + c_anf_matrix .* s_on));
%%
%[text] ## Lineare Nebenbedingungen
% Deckung der Nachfrage (3.1)
ap4.Constraints.demand = sum(p, 2) == demand()';

% Leistungsgrenzen (4.2)
ap4.Constraints.min = p_min_matrix .* v <= p;
ap4.Constraints.max = p <= p_max_matrix .* v;

% Anfahrtskosten (5.1)
ap4.Constraints.anfahrt = v - v_prev_matrix == s_on - s_off;

% Exklusivität (5.2)
ap4.Constraints.exklusiv = s_on + s_off <= 1;

% Ramp-Up (5.4)
ap4.Constraints.ramp_up = p_max_matrix >= p - p_prev_matrix - (su_matrix - p_max_matrix) .* v - (ru_matrix - su_matrix) .* v_prev_matrix;

% Ramp-Down (5.5)
ap4.Constraints.ramp_down = 0 <= p_max_matrix + p - p_prev_matrix + (rd_matrix - sd_matrix) .* v + (sd_matrix - p_max_matrix) .* v_prev_matrix;

% Shutdown (5.6)
ap4.Constraints.shutdown = p <= sd_matrix .* v + (p_max_matrix - sd_matrix) .* v_next_matrix;
%%
%[text] ## Lösung
solAP4 = solve(ap4);
%%
